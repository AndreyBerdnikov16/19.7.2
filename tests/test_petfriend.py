...
def test_post_create_new_pet_without_photo(name = 'Joshi', animal_type = 'Dog', age = '4'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
...
def test_update_pet_info(name='Iggy', animal_type='Dog', age=3):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_post_create_new_pet_with_photo(name = 'Joshiс', animal_type = 'Dog', age = '4', pet_photo = 'images/Cat_love.jpg'):
...
def test_post_create_new_pet_without_name(name = '', animal_type = 'Dog', age = '4'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.post_create_new_pet_without_photo(auth_key, name, animal_type, age)
    assert status == 403
    assert result['name'] == name
